...
LOG_MODULE_REGISTER(rm69310, CONFIG_DISPLAY_LOG_LEVEL);
...
/* RM69310 doesn't follow typical DBI behavior with D/C pin so we do this to make it fit in DBI */
static inline int rm69310_write_command(const struct device *dev, uint8_t cmd, const uint8_t *buf,
					size_t len)
{
	const struct rm69310_config *config = dev->config;
	int err;
...
static inline int rm69310_set_hardware_config(const struct device *dev)
{
	const struct rm69310_config *config = dev->config;
	int err;
	uint8_t tmp;
...
static int rm69310_resume(const struct device *dev)
{
	...
	err = rm69310_write_command(dev, RM69310_DISPLAY_ON, NULL, 0);
	if (err < 0) {
		return err;
	}
...
static int rm69310_suspend(const struct device *dev)
{
	const struct rm69310_config *config = dev->config;
	int err;

	err = rm69310_write_command(dev, RM69310_DISPLAY_OFF, NULL, 0);
	if (err < 0) {
		return err;
	}
	return mipi_dbi_release(config->mipi_dev, &config->dbi_config);
			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	err = rm69310_write_command(dev, RM69310_SET_ROW_ADDR, y_position, 2);
	if (err < 0) {
		return err;
	}
...
static int rm69310_set_pixel_format(const struct device *dev, const enum display_pixel_format pf)
{
...
static int rm69310_init_device(const struct device *dev)
{
	const struct rm69310_config *config = dev->config;
	int err;

	/* Turn display off */
	err = rm69310_suspend(dev);
	if (err < 0) {
		return err;
	}

	err = rm69310_set_hardware_config(dev);
	if (err < 0) {
		return err;
	}
	...
	err = rm69310_write_command(dev,
				    config->color_inversion ? RM69310_SET_REVERSE_DISPLAY
							    : RM69310_SET_NORMAL_DISPLAY,
				    NULL, 0);
	if (err < 0) {
		return err;
	}

	err = rm69310_resume(dev);
	if (err < 0) {
		return err;
	}
...
static int rm69310_init(const struct device *dev)
{
	...
	err = mipi_dbi_reset(config->mipi_dev, RM69310_RESET_DELAY);
	if (err < 0) {
		LOG_ERR("Failed to reset device!");
		return err;
	}

	err = rm69310_init_device(dev);
	if (err < 0) {
...
static DEVICE_API(display, rm69310_driver_api) = {
	.blanking_on = rm69310_suspend,
	.blanking_off = rm69310_resume,
	.write = rm69310_write,
	.set_contrast = rm69310_set_contrast,
...
DT_FOREACH_STATUS_OKAY(solomon_rm69310, RM69310_DEFINE_MIPI)
